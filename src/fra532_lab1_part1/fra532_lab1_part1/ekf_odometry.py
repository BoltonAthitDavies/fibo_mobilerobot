...
class EKFOdometryNode(Node):
    def __init__(self):
        ...
        self.covariance = np.eye(3) * 0.1  # Shared covariance for sequential sensor fusion
        
        # Process noise covariance (Q)
        # self.Q = np.diag([0.01, 0.01, 0.05]) 
        # self.Q = np.diag([0.001, 0.001, 0.001]) 
        # self.Q = np.diag([0.1, 0.1, 0.1]) 
        # self.Q = np.diag([0.9, 0.9, 0.9]) 
        self.Q = np.diag([0.05, 0.05, 0.02])
        
        # Measurement noise (R)
        # self.R_odom = np.diag([0.1, 0.1, 0.2]) 
        # self.R_odom = np.diag([5.0, 5.0, 5.0]) 
        # self.R_odom = np.diag([0.9, 0.9, 0.9]) 
        self.R_odom = np.diag([0.05, 0.05, 0.1])

        # self.R_imu = np.array([[0.05]]) # Scalar noise for omega_z
        # self.R_imu = np.array([[5.0]]) # Scalar noise for omega_z
        # self.R_imu = np.array([[0.001]]) # Scalar noise for omega_z
        # self.R_imu = np.array([[0.5]]) # Scalar noise for omega_z
        self.R_imu = np.array([[0.5]]) # Scalar noise for omega_z
        ...
        # Removed timer - prediction step is now called synchronously with measurements
        self.get_logger().info('EKF Odometry Node started')
        
    def joint_states_callback(self, msg):
        """Extract motion data for prediction and trigger Odom update."""
        self.get_logger().info('Joint states callback triggered')
        ...
        # FIXED: First do prediction step, then measurement update
        # 1. Prediction step first
        self.prediction_step()
        
        # 2. Then measurement update using the predicted state
        odom_z = np.array([
    ...
    def measurement_update_odom(self, z_measured):
        ...
        # Update state and covariance [cite: 224]
        self.state = self.state + K @ y
        self.covariance = (np.eye(3) - K @ H) @ self.covariance

    def imu_callback(self, msg):
        """Extract IMU angular velocity and perform update."""
        # Use angular velocity z as direct measurement
        self.measurement_update_imu(msg.angular_velocity.z)

    def measurement_update_imu(self, omega_z_measured):
        """Update using IMU angular velocity as heading validation.
        
        Sequential sensor fusion: IMU provides theta correction after wheel odometry.
        This is mathematically valid since measurements arrive at different times.
        """
        # Simple approach: Use IMU angular velocity to validate/constrain 
        # the theta component of the state when it differs significantly
        # from wheel odometry predictions
        
        # Only apply IMU correction if we have significant angular motion
        if abs(omega_z_measured) < 0.01:  # Threshold for meaningful rotation
            return
            
        # For simplicity, treat this as a "virtual measurement" that constrains
        # the heading when the robot is turning. The IMU provides independent
        # information about rotation that can correct wheel slippage effects.
        
        # Virtual measurement: assume IMU gives us a constraint on theta evolution
        # We'll use a simplified approach where the IMU measurement acts as
        # a soft constraint on the current heading estimate
        
        # H matrix: relates state to "virtual heading rate" measurement
        H = np.array([[0, 0, 1]])
        
        # Create a "corrected theta" estimate based on IMU
        # This is not integration - just a correction factor
        omega_correction_factor = 0.1  # Small correction weight
        theta_imu_correction = self.state[2] + omega_correction_factor * omega_z_measured
        
        # Wrap the corrected angle
        theta_imu_correction = math.atan2(math.sin(theta_imu_correction), 
                                         math.cos(theta_imu_correction))
        
        z_measured = np.array([theta_imu_correction])
        
        # Innovation
        y = z_measured - np.array([self.state[2]])
        ...
        # Update state and covariance (sequential fusion with wheel odom)
        self.state = self.state + K @ y
        ...
        self.get_logger().info(f'IMU update: omega_z={omega_z_measured:.3f}, theta_correction={y[0]:.3f}, final_theta={self.state[2]:.3f}')

    def publish_odometry(self):
        """Publish current EKF state as odometry message."""        
        self.get_logger().info(f'Publishing EKF odom: [{self.state[0]:.3f}, {self.state[1]:.3f}, {self.state[2]:.3f}]')        
        current_time = self.get_clock().now()
        odom_msg = Odometry()
        odom_msg.header.stamp = current_time.to_msg()
        odom_msg.header.frame_id = 'odom'
        odom_msg.child_frame_id = 'ekf_odom'
    ...
    def publish_transform(self, timestamp, theta):
        ...
        tf_msg.child_frame_id = 'ekf_odom'
